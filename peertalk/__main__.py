...
class MainWindow(wx.Frame):
    def __init__(self, parent, app):
        ...
        # DO NOT TOUCH! 
        self.app = app
        wx.Frame.__init__ ( self, parent, id = wx.ID_ANY, title = "Peertable", pos = wx.DefaultPosition, size = wx.Size( 1007,568 ), style = wx.DEFAULT_FRAME_STYLE|wx.TAB_TRAVERSAL )
        self.SetSizeHints( wx.DefaultSize, wx.DefaultSize )
        ...
        self.buffer.SetFont( wx.Font( 9, wx.FONTFAMILY_MODERN, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL, False, "Courier New" ) )
    ...
    def connect(self, event):
        ...
        self.ipCtrl.SetValue('')
        print("Connecting to: {}".format(addr))
        self.app.server.connect(addr)
...
class WxChatApp(pt.ChatApp):
    ...
    def post_dialog(self):
        ...
        print("Starting peer...")
        print(" o  Listen port: {}".format(port))
        print(" o  Public port: {}".format(eport or port))
        print(" o  Public address: {}".format(addr))
        print(" o  Nickname/pseudonym: {}".format(nick))
    ...
    def on_app_message(self):
        while not self.messages.empty():
            ...
            print("* Adding to buffer: '" + "', '".join(str(x) for x in m) + "'")
            ...
            if mtype == "PMESSAGE":
                ...
                if m[3] in self.nicknames:
                    self.frame.userList.SetValue(self.frame.userList.GetValue().replace("{}\n".format(self.nicknames[m[3]]), ''))
                
                else:
                    self.frame.userList.SetValue(self.frame.userList.GetValue().replace("[{}]\n".format(m[3]), ''))
                
            elif mtype == "JOIN":
                self.frame.buffer.SetValue(self.frame.buffer.GetValue() + "\n+++ ID {} has joined".format(m[3]))
                
                if m[3] in self.nicknames:
                    self.frame.userList.SetValue(self.frame.userList.GetValue() + "{}\n".format(self.nicknames[m[3]]))
                
                else:
                    self.frame.userList.SetValue(self.frame.userList.GetValue() + "[{}]\n".format(m[3]))
                
            elif mtype == "NICK":
                if m[3] in self.nicknames and m[4] is not None:
                    self.frame.buffer.SetValue(self.frame.buffer.GetValue() + "\n*** {} is now known as {}".format(m[4], m[5]))
                    
                else:
                    self.frame.buffer.SetValue(self.frame.buffer.GetValue() + "\n*** ID {} is now known as {}".format(m[3], m[5]))
                    
                if m[3] in self.nicknames:
                    self.frame.userList.SetValue(self.frame.userList.GetValue().replace("{}\n".format(self.nicknames[m[3]]), m[5] + "\n"))
